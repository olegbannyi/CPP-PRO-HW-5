...
all-safe-ref: clean-safe build-safe run-safe-ref

run-safe-ref: $(BUILD_SAFE_DIR)/banking.exe
	./$(BUILD_SAFE_DIR)/banking.exe --ref
run-safe: $(BUILD_SAFE_DIR)/banking.exe
	./$(BUILD_SAFE_DIR)/banking.exe

build-safe: safe.o core.o
	$(CXX) $(CXXFLAGS) $(BUILD_SAFE_DIR)/*.o $(BUILD_CORE_DIR)/*.o -o $(BUILD_SAFE_DIR)/banking.exe
safe.o:
...
clean-safe:
	rm -f $(BUILD_SAFE_DIR)/*.o $(BUILD_SAFE_DIR)/*.exe  $(BUILD_SAFE_DIR)/*.log


# unsafe mode
all-unsafe: clean-unsafe build-unsafe run-unsafe
all-unsafe-ref: clean-unsafe build-unsafe run-unsafe-ref

run-unsafe-ref: $(BUILD_UNSAFE_DIR)/banking.exe
	./$(BUILD_UNSAFE_DIR)/banking.exe --ref
run-unsafe: $(BUILD_UNSAFE_DIR)/banking.exe
	./$(BUILD_UNSAFE_DIR)/banking.exe
build-unsafe: unsafe.o core.o
	$(CXX) $(CXXFLAGS) $(BUILD_UNSAFE_DIR)/*.o $(BUILD_CORE_DIR)/*.o -o $(BUILD_UNSAFE_DIR)/banking.exe
unsafe.o:
...
clean-unsafe:
	rm -f $(BUILD_UNSAFE_DIR)/*.o $(BUILD_UNSAFE_DIR)/*.exe $(BUILD_UNSAFE_DIR)/*.log
